    ...
    randomResult: RandomResult) -> SpatialRestirResult
{
    ...
    for(var iSample: u32 = 0u; iSample < iNumSamples; iSample++)
    {
        ...
        neighborTemporalRadiance.x *= neighborTemporalReservoir.w;
        neighborTemporalRadiance.y *= neighborTemporalReservoir.w;
        neighborTemporalRadiance.z *= neighborTemporalReservoir.w;
    ...
    // weight
    ret.mReservoir.w = clamp(ret.mReservoir.x / (fNumValidSamples * ret.mReservoir.y + 0.0001f), 0.0f, 1.0f);
...
fn updateSpatialReservoir(
    ...
    fJacobian: f32) -> SpatialRestirResult
{
    var ret: SpatialRestirResult = result; 
    ret.mRandomResult = randomResult;
    ret.mRandomResult = nextRand(ret.mRandomResult.miSeed);
